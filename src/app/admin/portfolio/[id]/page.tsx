...
export default function AdminPortfolioEditPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const loadItem = async () => {
    ...
    if (!data) { router.push("/admin/portfolio"); return; }
    ...
    setM2Total(data.m2_total ? String(data.m2_total) : "");
  ...
  const updateGalleryImage = (i: number, url: string) => {
    const updated = [...gallery]; updated[i] = url; setGallery(updated);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error: updateError } = await supabase.from("portfolio_items").update({
      title: title.trim(),
      slug: slug.trim(),
    ...
    if (updateError) { setError(updateError.message); toast.error("Error al guardar portfolio"); setSaving(false); return; }
    ...
    return (
      <AdminShell>
        <div className="flex items-center justify-center py-20">
          <p className="text-brand-gray-dark">Cargando proyecto…</p>
        </div>
      </AdminShell>
    );
  }
    ...
    <AdminShell>
      <div className="flex flex-col gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-[var(--font-heading)] text-2xl font-bold text-brand-black">Editar proyecto</h1>
          ...
          <div className="space-y-6 lg:col-span-2">
            <div className="rounded-lg border border-brand-gray bg-white p-5">
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <input type="number" min={0} step="0.01" value={m2Total} onChange={(e) => setM2Total(e.target.value)}
                    className="mt-1 w-full rounded-lg border border-brand-gray bg-white px-3 py-2 text-sm outline-none focus:border-brand-red" />
                </div>
                <div className="sm:col-span-2">
            ...
            <div className="rounded-lg border border-brand-gray bg-white p-5">
              ...
              <div className="mt-4 space-y-2">
                ...
                {gallery.length === 0 && (
                  <p className="py-4 text-center text-sm text-brand-gray-dark">Sin imágenes</p>
                )}
              </div>
            </div>
          </div>
            ...
            <div className="rounded-lg border border-brand-gray bg-white p-5">
              ...
              <div className="mt-3 flex gap-2">
                ...
                <button type="button" onClick={addTag}
                  className="rounded-lg bg-brand-cream px-3 py-2 text-sm font-medium text-brand-black hover:bg-brand-gray">+</button>
              </div>
              {tags.length > 0 && (
